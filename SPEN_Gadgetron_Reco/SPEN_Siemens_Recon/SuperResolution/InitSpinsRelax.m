function spins = InitSpinsRelax(chemShiftVec, ...
                                ...
                                sampleOffset, ...
                                B1)
%   spins = InitSpinsRelax(chemShiftVec,numSpinsPerShift,sampleSize,initialMag,T1,T2,eqMag)
%
...
if (numSpinsPerShift>1)
    % zz = [-sampleSize/2:dz:sampleSize/2-dz];
    zz = linspace(-sampleSize/2, sampleSize/2, numSpinsPerShift);
else
    zz = 0;
end
...
for curShift=1:numShifts
    for curSpin=1:numSpinsPerShift
        counter = counter + 1;
        spins(counter).r  = [0; 0; zz(curSpin) + sampleOffset];
