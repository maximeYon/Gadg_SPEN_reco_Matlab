function [L2image] = function_OneShotReconstruction(CmplxDataSh,gccmtx_aligned_finalx,nccx,FinalAFull,parameters,pFT)
...
%    CCCmplxDataSx(:,1(Nro*pFT+1):end,:,:,:,:,:)=0;
    CCCurCmplxDataSx(:,1:ceil((1-pFT)*Nro),:,:,:,:,:)=0;
    for SliI=1:nSlices
        for bval=1:nWeight
            for i=1:nccx
        [~, CCCurCmplxDataSx(:,:,i,SliI,:,bval)] = pocs(CCCurCmplxDataSx(:,:,i,SliI,:,bval), 20, 0);
            end
        end
    end
%    CCCmplxDataS1(:,(Nro*pFT+1):end,:,:,:,:,:)=0;
    CCCurCmplxDataS1(:,1:ceil((1-pFT)*Nro),:,:,:,:,:)=0;
    for SliI=1:nSlices
        for bval=1:nWeight
    [~, CCCurCmplxDataS1(:,:,:,SliI,:,bval)] = pocs(CCCurCmplxDataS1(:,:,:,SliI,:,bval), 20, 0);
        end
    end
end
